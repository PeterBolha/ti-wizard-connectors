...
class LocalConfigVersionManager(ConfigVersionManager):
    def __init__(self, local_version_manager_cfg):
        ...
        self.__CONFIG_FILE_NAME = local_version_manager_cfg.get("config_file_name")
        ...
        try:
            ...
            raise ValueError(
                f"Invalid config file format '{config_file_format_in_cfg}' in configuration. "
                f"Allowed values are: {', '.join([e.name for e in ConfigFileFormat])}"
            )
